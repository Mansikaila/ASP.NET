...
//    }
    ...
    if ($main_id) {
        $sql = "SELECT od.outward_detail_id,od.outward_id,im.inward_id,od.inward_detail_id,od.out_qty,od.out_wt,od.loading_charges,id.lot_no,id.inward_qty AS stock_qty, um.packing_unit_name AS packing_unit, id.location,im.inward_no,DATE_FORMAT(im.inward_date, '%d-%m-%Y') AS inward_date,itm.item_name AS item,id.variety FROM tbl_outward_detail od LEFT JOIN tbl_inward_detail id ON od.inward_detail_id = id.inward_detail_id LEFT JOIN tbl_inward_master im ON id.inward_id = im.inward_id LEFT JOIN tbl_item_master itm ON id.item = itm.item_id LEFT JOIN tbl_packing_unit_master um ON id.packing_unit = um.packing_unit_id  WHERE od.outward_id = :outward_id ORDER BY od.outward_detail_id ASC";
