...
if (isset($_POST['ajax_get_lots']) && isset($_POST['customer_id'])) {
    try {
        global $_dbh;
    ...
    if ($companyYearId) {
        ...
        if ($yearRow) {
            ...
            $next_invoice_sequence = (isset($seqRow['max_seq']) && is_numeric($seqRow['max_seq']))
                ? $seqRow['max_seq'] + 1 : 1; 
            $sequence_padded = str_pad($next_invoice_sequence, 4, '0', STR_PAD_LEFT); // Pad sequence with leading zeros
            $invoice_no_formatted = $sequence_padded . '/' . $finYear; // Format invoice_no with financial year
        }
    }
...
<script>
// autofocus-tax invoice and sgst-cgst-igst disabled,use regular - hsn code disabled and Remove layout,
    //use bill of supply Tax Amount,sgst-cgst-igst disabled - drashti 
...
document.getElementById("generate").addEventListener("click", function () {
    ...
    var lot_no = document.getElementById('lot_no') ? document.getElementById('lot_no').value : '';
    var customer = document.getElementById('customer') ? document.getElementById('customer').value : '';
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.error) {
                    ...
                    options = '<option value="">Error loading lots</option>';
                } else if (response && response.length > 0) {
                    $.each(response, function(i, lot) {
                        options += '<option value="' + lot.lot_no + '">' + lot.lot_no + '</option>';
                    });
                } else {
                    options += '<option value="">No lots found</option>';
                }
                $('#lot_no').html(options);
            },
            error: function(xhr, status, error) {
                console.error("AJAX Error:", status, error);
                $('#lot_no').html('<option value="">Error loading lots</option>');
            }
        });
    });
    if ($('#customer').val()) {
        ...
        $('#lot_no').html('<option value="">Select Lot No</option>');
    }
});
</script>-->
<script>
document.addEventListener("DOMContentLoaded", function() {
  // On customer change, fetch lots for that customer
  ...
  if (customerInput) {
    customerInput.addEventListener('change', function() {
      ...
      fetch(window.location.href, {
        ...
        body: new URLSearchParams({ ajax_get_lots: 1, customer_id: customerId })
      })
