          ...
          <div class="form-group row gy-2">
            ...
            if ($row) {
              ...
              if ($generator_options) {
                ...
                // Standard rendering invoice_type
if ($fieldname == "invoice_type") {
    ...
    $invoiceType = isset($_bll->_mdl->_invoice_type) ? $_bll->_mdl->_invoice_type : '2'; // Default to Tax Invoice
    echo '<div class="form-check form-check-inline m-0">';
    echo '<input class="form-check-input" type="radio" name="invoice_type" value="1" id="invoice_type1"' . ($invoiceType == '1' ? ' checked' : '') . '>';
    echo '<label class="form-check-label" for="invoice_type1">Regular</label>';
    echo '</div>';
    echo '<div class="form-check form-check-inline m-0">';
    echo '<input class="form-check-input" type="radio" name="invoice_type" value="2" id="invoice_type2"' . ($invoiceType == '2' ? ' checked' : '') . '>';
    echo '<label class="form-check-label" for="invoice_type2">Tax Invoice</label>';
    echo '</div>';
    echo '<div class="form-check form-check-inline m-0">';
    echo '<input class="form-check-input" type="radio" name="invoice_type" value="3" id="invoice_type3"' . ($invoiceType == '3' ? ' checked' : '') . '>';
...
?>
    ...
    function initializeEditMode() {
        ...
        if (transactionMode === 'U') {
            // Get invoice_type from model (set by PHP)
            const invoiceType = '<?php echo isset($_bll->_mdl->_invoice_type) ? $_bll->_mdl->_invoice_type : ''; ?>';
            if (invoiceType) {
                $(`input[name="invoice_type"][value="${invoiceType}"]`).prop('checked', true);
                handleInvoiceTypeChange();
                updateHSNCodeByInvoiceType();
            }
            // Get invoice_for from model
            const invoiceFor = '<?php echo isset($_bll->_mdl->_invoice_for) ? $_bll->_mdl->_invoice_for : ''; ?>';
            if (invoiceFor) {
                $('#invoice_for').val(invoiceFor);
                toggleManualInvoiceDetails();
            }
            // Fetch lots for the selected customer
            const customerId = '<?php echo isset($_bll->_mdl->_customer) ? $_bll->_mdl->_customer : ''; ?>';
            if (customerId && invoiceType) {
                fetchLots();
            }
        }
    }
    // Call initialization for edit mode
    initializeEditMode();
    // Existing code for invoice_type handling
    function handleInvoiceTypeChange() {
        ...
        // Trigger lot fetch on invoice_type change
        fetchLots();
    }
        ...
        fetch(window.location.href, {
            ...
            .then(response => {
                if (response.error) {
                    ...
                    // In edit mode, select the lot_no values from the model
                    if (transactionMode === 'U') {
                        const lotNo = '<?php echo isset($_bll->_mdl->_lot_no) ? $_bll->_mdl->_lot_no : ''; ?>';
                        if (lotNo) {
                            const lotNos = lotNo.split(',');
                            lotNos.forEach(lot => {
                                $(`input[name="lot_no[]"][value="${lot.trim()}"]`).prop('checked', true);
                            });
