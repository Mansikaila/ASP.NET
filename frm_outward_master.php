    ...
    fetch('pending_inward.php?customer=' + encodeURIComponent(customerId))
        ...
        .then(data => {
            ...
            data.forEach(row => {
                ...
                tr.innerHTML = `
                    <td><input type="checkbox" class="select-inward-checkbox"></td>
                    ...
                    <td data-label="Out Qty" class="out-qty-cell">${row.out_qty ?? 0}</td>
                    <td data-label="Out Wt" class="out-wt-cell">${row.out_wt ?? 0}</td>
                    <td data-label="Loading Charges" class="loading-charge-cell">${row.loading_charge ?? 0}</td>
...
// Live Out Wt calculation as you change Out Qty
