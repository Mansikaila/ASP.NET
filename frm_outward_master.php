...
function updateTableRow(index, rowData) {
 ...
 document.getElementById("btn_add").addEventListener("click", function (event) {
    ...
    checkedRows.forEach(cb => {
        ...
        // Prepare the detail record object. Make sure key names match your detail model and grid columns
        const record = {
            ...
            out_qty: tr.querySelector('[data-label="Out Qty"]').textContent.trim() || "0",
            unit: tr.querySelector('[data-label="Unit"]').textContent.trim(),
            out_wt: tr.querySelector('[data-label="Out Wt"]').textContent.trim() || "0",
            loading_charges: tr.querySelector('[data-label="Loading Charges"]').textContent.trim() || "0",
            location: tr.querySelector('[data-label="Location"]').textContent.trim(),
            detailtransactionmode: 'I'
        };
        // Prevent duplicates (by inward_detail_id)
...
<script>
// Temporary storage for user-edited values before save
let editedInwardData = {};

// Function to store edited values for later retrieval
...
document.getElementById('pendingInwardTableBody').addEventListener('input', function(e) {
    ...
    if (e.target.classList.contains('out-qty-cell')) {
        ...
        // Save to edited data if loading charge changes
        saveEditedInwardRow(tr);
    }
});
