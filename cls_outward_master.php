...
{   
    ...
    public function getForm($transactionmode="I",$label_classes="col-12 col-sm-3 col-md-2 col-lg-2 col-xl-2 col-xxl-1", $field_classes="col-12 col-sm-6 col-md-4 col-lg-4 col-xl-3 col-xxl-2") {
        $next_outward_sequence = isset($GLOBALS['next_outward_sequence']) ? $GLOBALS['next_outward_sequence'] : '';
        $outward_no_formatted = isset($GLOBALS['outward_no_formatted']) ? $GLOBALS['outward_no_formatted'] : '';
        $output=""; $hidden_str="";
        ...
        $total_qty_index = array_search('total_qty', $this->_mdl->generator_fields_names);
        if(is_array($this->_mdl->generator_fields_names) && !empty($this->_mdl->generator_fields_names)){
            ...
            {
                   if (
                        $fieldname == "outward_sequence" ||
                        $fieldname == "outward_no" ||
                        ($total_qty_index !== false && $i >= $total_qty_index)
                    ) {
                ...
                switch($this->_mdl->generator_fields_types[$i]) {
                    ...
                    case "select":
                        $value="";$field_str="";$cls="";$flag=0;
                            ...
                            if($this->_mdl->generator_fields_types[$i]=="checkbox" || $this->_mdl->generator_fields_types[$i]=="radio") {
                                ...
                                $chk_str="";if (($fieldname == "outward_sequence" || $fieldname == "outward_no") && $transactionmode != "U") {
                                if ($fieldname == "outward_sequence") {
                         ...
                         if ($fieldname == 'outward_date' && empty($value)) {
                                        $value = '';  
                                            }
                                        
                                        if($fieldname == 'outward_date') {
                            ...
                            if (
                                ...
                                $dropdown_html = getDropdown($this->_mdl->generator_dropdown_table[$i],$this->_mdl->generator_value_column[$i],$this->_mdl->generator_label_column[$i],
                                $where_condition_val,$fieldname,$selected_val,$cls,$required_str);
                 ...
                 if(empty($this->_mdl->generator_after_detail) || (!empty($this->_mdl->generator_after_detail) && !in_array($fieldname,$this->_mdl->generator_after_detail))) {
                    ...
                    $output.=$lbl_str;
                    if($field_str) {
                        $extra_margin_class = ($fieldname == 'outward_date') ? ' mt-3' : '';
                        $output.='<div class="'.$field_layout_classes.$extra_margin_class.'">';
                        $output.=$field_str;
                        $output.='</div>';
                    }
                    if($table_layout=="vertical" && $this->_mdl->generator_fields_types[$i]!="hidden") {
                ...
                <div class="modal-body">
                   <table class="table table-bordered table-striped table-sm align-middle" style="width:100%;">
                        ...
                        while (\$row = \$stmt->fetch(PDO::FETCH_ASSOC)) {
                            "<tr data-inward-id='" . htmlspecialchars(\$row['inward_id'] ?? 0) . "' data-inward-detail-id='" . htmlspecialchars(\$row['inward_detail_id'] ?? 0) . "'>";
